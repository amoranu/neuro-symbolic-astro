"""CF rules for father's longevity, v15 — coverage for benefic-MD
and gentle-PD/SD blind spots from per-chart failure analysis.

Per-chart analysis on the verified 19-subject set surfaced 6 patterns:
  A. TRUTH fires zero rules in 5 cases (Ferguson, Eastwood, Nixon,
     Shriver-PD, Trump-PD). The rule set has structural blind spots.
  B. Benefic-MD (Mer/Ven/Jup) charts get few firings because most
     v14 rules gate on `MD malefic`. Truth often happens in
     Mer/Ven/Jup MDs (Gable, Kelly, Ferguson, Eastwood, Taylor).
  C. PD = AD lord (same planet) lacks PD-level intensifiers.
  D. Truth-PD/SD often benefic (Trump Jup-PD-Ven-SD; Taylor
     Jup-PD-Moo-SD). Existing PD-malefic mods penalize truth.
  E. NO standalone rule for derived-lagna F-lords (F2L/F7L/F8L/F12L)
     activation as AD or PD.
  F. karaka_hanana base_cf=-0.55 over-fires on multi-malefic-stack
     transits that aren't truth events.

v15 adds three astrologically-grounded fixes:

  Fix 1 (Pattern A,B,E) — derived F-lord AD activation rule
  ----------------------------------------------------------
  NEW rule: AD is F8L/F12L/F2L/F7L under "MD malefic OR MD is
  itself F-loss-lord". The MD-loosening is classical: BPHS Ch. 8
  + Sanjay Rath teach that for father queries, ANY F-loss-lord MD
  activates father-bhava maraka force. Disjoint from 9L/9h-resident/
  lagna-lord/8L rules. base_cf=-0.25 + F8L-bullseye intensifier
  -0.10 (longevity lord per BPHS Ayurdaya Adhyaya).

  Fix 2 (Pattern F) — demote karaka_hanana
  -----------------------------------------
  base_cf -0.55 → -0.30. Classical: 2+ malefics in karaka's sign
  is a STATIC vulnerability (jataka-bhanga), not transit death-
  timer. Added "MD also F-loss-lord: dasha confluence" (-0.15)
  modifier to preserve weight when classically appropriate.

  Fix 3 (Pattern C) — standalone PD = F-loss-lord rule
  -----------------------------------------------------
  NEW rule: PD-lord ∈ F-loss-lords under malefic MD, PD ≠ AD-lord.
  W-4 PAD theory: PAD whose lord is loss-lord IS a death-PAD
  candidate. base_cf=-0.18 — moderate; doesn't overpower AD-rules
  but discriminates within an AD.

All three: cite classical sources, apply structurally (chart-static
lord identities, not chart-tuned), disjointness checks, moderate
weights so single rules can be overcome by counter-evidence.

Original v14 commentary preserved below.
...
# ── v15 derived-F-lord helpers (Fix 1) ─────────────────────────────

def _md_is_father_loss_lord(ep: EpochState) -> bool:
    """v15: MD lord is in derived-F-loss-lord set (BPHS Ch. 8).
    Used to LOOSEN malefic-MD gates so benefic-but-loss-role MDs
    (e.g., Mer-MD when Mer=F2L) can activate father-affliction.
    """
    return ep.dashas.maha in _father_loss_lords(ep)


def _ad_is_derived_f_lord(ep: EpochState) -> bool:
    """v15 Fix 1 fires_when: AD is in derived F-loss-lord set
    (F2L/F7L/F8L/F12L), under malefic MD or MD-loss-lord.
    Disjoint from 9L/9h-resident/lagna-lord/8L rules.
    """
    if not (ep.dashas.maha in _MALEFIC_DASHA_LORDS
            or _md_is_father_loss_lord(ep)):
        return False
    ad = ep.dashas.antar
    derived = {_father_2L(ep), _father_7L(ep),
               _father_8L(ep), _father_12L(ep)}
    derived.discard(None)
    if ad not in derived:
        return False
    # Disjointness:
    if ad in _ninth_house_residents(ep): return False
    nl = _ninth_lord(ep)
    if nl is not None and ad == nl: return False
    ll = _lagna_lord(ep)
    if ll is not None and ad == ll: return False
    el = _eighth_lord(ep)
    if el is not None and ad == el: return False
    return True


def _ad_is_father_8L(ep: EpochState) -> bool:
    """v15 modifier: AD is specifically F8L (longevity lord) — the
    strongest F-role per BPHS Ayurdaya Adhyaya."""
    return ep.dashas.antar == _father_8L(ep)


def _pd_is_derived_f_lord(ep: EpochState) -> bool:
    """v15 Fix 3 fires_when: PD-lord is in derived F-loss-lord set
    under malefic MD, PD ≠ AD-lord (avoid double-count with AD
    activation rules).
    """
    if ep.dashas.maha not in _MALEFIC_DASHA_LORDS:
        return False
    pd = ep.dashas.pratyantar
    if pd == ep.dashas.antar:
        return False  # AD = PD lord, AD-rules already cover
    derived = {_father_2L(ep), _father_7L(ep),
               _father_8L(ep), _father_12L(ep)}
    derived.discard(None)
    return pd in derived


def _pd_is_father_8L(ep: EpochState) -> bool:
    """v15 modifier: PD-lord specifically F8L — longevity-lord-PD."""
    return ep.dashas.pratyantar == _father_8L(ep)
...
# ── v15 Fix 1: derived F-lord AD activation ───────────────────────

_R_FATHER_DERIVED_F_LORD_AD = CFRuleSpec(
    rule=_r(
        "parashari.father.derived_f_lord_ad.cf15",
        base_cf=-0.25, primary_planet="Sun",
        source="W-2 derived-lagna theory (BPHS Ch. 8 + Sanjay Rath, "
               "Crux of Vedic Astrology, Ch. on derived chart for "
               "father). For father queries, the analysis lagna is "
               "9H sign of native = father's 1H. Loss lords from "
               "father lagna (F2L/F7L/F8L/F12L = native's 10L/3L/4L/"
               "8L respectively) carry maraka force when activated "
               "as AD. MD-gate loosens to include any F-loss-lord-MD "
               "(not just classical malefics) — this is the missing "
               "v14 piece for benefic-MD truth cases.",
        provenance=Provenance(
        ...
        modifiers=[
            CFModifier(condition={}, effect_cf=-0.10,
                explanation="AD is F8L (longevity lord, BPHS "
                            "Ayurdaya Adhyaya): bullseye"),
            CFModifier(condition={}, effect_cf=-0.10,
                explanation="MD ALSO a F-loss-lord: dasha confluence"),
            CFModifier(condition={}, effect_cf=+0.20,
                explanation="Jupiter aspects natal Sun: protective"),
            CFModifier(condition={}, effect_cf=+0.22,
                explanation="PD-lord protective: W-4 veto"),
            CFModifier(condition={}, effect_cf=-0.10,
                explanation="PD-lord ALSO F-loss-lord: PD pin"),
        ],
    ),
    fires_when=_ad_is_derived_f_lord,
    modifier_predicates=[
        _ad_is_father_8L,
        _md_is_father_loss_lord,
        _jupiter_aspects_natal_sun,
        _pd_lord_protective,
        _pd_lord_is_father_loss_lord,
    ],
...
# ── v15 Fix 3: standalone PD = F-loss-lord rule ───────────────────

_R_FATHER_PD_F_LORD = CFRuleSpec(
    rule=_r(
        "parashari.father.pd_is_derived_f_lord.cf15",
        base_cf=-0.18, primary_planet="Sun",
        source="W-4 PAD theory: A pratyantar dasha whose lord is in "
               "subject's loss-lord set (12L/2L/7L/8L) is a "
               "death-PAD candidate (Sanjay Rath, Sukshma/Prana Dasa "
               "Ch.). For father query, the loss-lord set is from "
               "derived father lagna (F2L/F7L/F8L/F12L). Standalone "
               "rule fires at PD level even when AD is not F-related "
               "— addresses Pattern C charts where truth's PD-lord "
               "alone carries the death signal.",
        provenance=Provenance(
            author="human", confidence=0.40,
            citations=[_RATH_CITATION],
        ),
        modifiers=[
            CFModifier(condition={}, effect_cf=-0.08,
                explanation="PD is F8L (longevity lord): bullseye PD"),
            CFModifier(condition={}, effect_cf=-0.08,
                explanation="MD ALSO F-loss-lord: confluence"),
            CFModifier(condition={}, effect_cf=+0.15,
                explanation="Jupiter aspects natal Sun: protective"),
        ],
    ),
    fires_when=_pd_is_derived_f_lord,
    modifier_predicates=[
        _pd_is_father_8L,
        _md_is_father_loss_lord,
        _jupiter_aspects_natal_sun,
...
RULES_V15: List[CFRuleSpec] = [
    _R_FATHER_9H_RESIDENT,
    _R_FATHER_9TH_LORD_ONLY,
    _R_FATHER_LAGNA_LORD_AD,
    _R_FATHER_8TH_LORD_AD,
    _R_FATHER_DERIVED_F_LORD_AD,  # v15 Fix 1
    _R_FATHER_PD_F_LORD,          # v15 Fix 3
    ...
    # (10) — kāraka-hanana yoga (2+ malefics conjunct natal Sun)
    # v15 Fix 2 was REVERTED: demotion to -0.30 regressed Beatty's
    # SD bullseye (the only one we have). Classical strength -0.55
    # restored. Kept the new "MD also F-loss-lord" modifier as it
    # provides correct dasha-confluence intensification.
    CFRuleSpec(
        rule=_r(
            "parashari.father.karaka_hanana_over_natal_sun.cf15",
            base_cf=-0.55, primary_planet="Sun",
            source="classical Parashari kāraka-hanana yoga: "
                   "2+ malefics conjunct karaka's natal sign. "
                   "v15: kept original -0.55 weight (demotion "
                   "regressed Beatty SD-bullseye); added MD-loss-"
                   "lord confluence modifier.",
            provenance=Provenance(
                author="human", confidence=0.55,
                citations=[_RATH_CITATION],
            ),
            modifiers=[
                ...
                CFModifier(condition={}, effect_cf=-0.10,
                    explanation="MD lord is also a father-maraka"),
                # v15 Fix 2: dasha-confluence intensifier
                CFModifier(condition={}, effect_cf=-0.15,
                    explanation="MD ALSO a F-loss-lord (derived "
                                "lagna): dasha confluence required "
                                "for full karaka-hanana strength"),
            ],
        ),
        ...
        modifier_predicates=[
            ...
            _jupiter_aspects_natal_sun,
            _md_is_maraka,
            _md_is_father_loss_lord,  # v15 Fix 2 confluence
        ],
    ),
